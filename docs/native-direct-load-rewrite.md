...
- Darker Color and Lighter Color compare source/destination with Rec.709 luma
  ...
  (`max-min <= 2/255`) to min/max channel membership. Color uses a rounded
  byte-domain `0.3/0.59/0.11` luminosity path only for the low-side ClipColor
  branch; ordinary/high-clamp Color stays on `0.3/0.6/0.1`. Saturation additionally
