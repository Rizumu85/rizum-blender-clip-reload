...
- High-leverage native performance work has moved from broad investigation to a
  ...
  compressed raster tile chunks, shader-side mask atlas events, and a clipped
  raster sibling tile-silo subset inside existing clipping caches.
  ...
  richer clipping/base tile events and larger tile-local raster/clipping
  ...
  extend tile-local clipping/base events on the native side, and use Blender
