...
- Streaming clipping runs now bound clipped preserve passes by the base cache dirty rect. Metadata-known clipped siblings that cannot intersect the base cache are skipped before provider raster decode/upload, and overlapping clipped siblings no longer expand clipping-cache dirty bounds to their own source bounds. This is a scheduling/resource optimization only; the blend and alpha-preserve shaders are unchanged. The remaining large-stack performance blocker is still non-empty full-canvas intermediate cache allocation/lifetime plus visible-tile inflate and GPU pass throughput.
