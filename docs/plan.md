...
- Current native milestone: continue strict GPU coverage for raster adjustment/filter layers, remaining byte-domain blend quantization, and large-stack GPU performance. Ordinary raster blend modes `LayerComposite=1..26` plus `36` are enabled, isolated containers can resolve with supported non-NORMAL blend modes, clipping runs support non-NORMAL raster bases plus clipped raster siblings, THROUGH groups clear the clip base for following clipped layers, and LUT-style adjustment/filter layers now route through a dedicated GPU pass: Brightness/Contrast (`FilterLayerInfo` type `1`), Level Correction (`2`), Tone Curve (`3`), Invert/Reverse Gradient (`6`), Posterization (`7`), and Gradient Map (`9`). HSL (`4`), Color Balance (`5`), and Threshold (`8`) remain explicit unsupported filter work until faithful native models exist. `IllustrationBlendModes.clip`, `IllustrationBlendModes2.clip`, `Test_AddGlowMultiply.clip`, `Test_ToneCurve.clip`, `Test_Gradiation.clip`, and `Test_RealArt.clip --gpu-support-check` are fully routed but still have residual formula/quantization or performance work; improve correctness only with source-backed native evidence and guard samples.
