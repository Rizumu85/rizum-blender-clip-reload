...
- Latest clipped-sibling performance update: direct raster-only child clipping runs inside clipped container/folder sibling child streams now lower through a second local clip-base accumulator, and simple THROUGH children now lower by resolving the before/after accumulator back into the clipped sibling scope, including the position where that THROUGH child sits inside one nested simple container. Deeper THROUGH nesting and non-raster child subtrees inside clipped siblings remain barriers.
