...
- HSL filter payload mapping is now sample-backed separately from the native
  per-pixel routine: the SQLite payload uses UI-degree hue, fixed-point
  saturation, and UI-percent luminosity for `Test_HSL`, while the GPU shader
  implements CSP's luminosity/saturation coupling from the native routine.
