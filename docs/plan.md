...
- Current native milestone: the stock Blender image-datablock bridge is the
  ...
  metadata. `tools/build_blender_addon.py` builds the installable zip with
  `__init__.py`, `native_bridge.py`, and the locally built release `clip_capi`
  library under `clip_studio_importer/native/`; it no longer packages the Python
  compositor/loader. The native library preference is an override only. The
  project-root `clip_loader.py` remains slow reference tooling for verification
  while native fidelity gaps close.
