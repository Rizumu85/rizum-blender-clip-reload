...
- The performance roadmap is now frozen and short in `docs/native-tile-event-renderer-roadmap.md`; detailed Forty-* implementation logs live in `docs/native-tile-event-renderer-changelog.md`. The compact target model is `docs/native-tile-event-main-execution-plan.md`, the implementation strategy is `docs/native-tile-event-architecture-strategy.md`, the module seam rules are in `docs/native-tile-event-module-boundaries.md`, and the measured barrier/performance gate is `docs/native-tile-event-scoreboard.md`. Tile-local event execution is the main renderer model, while barrier passes are explicit, counted, and explained as not-yet-lowered faithful semantics. Future work should add render segment kinds and typed event executor support rather than more opportunistic traversal branches, and must prove a `planned_passes` or `barrier_segments` reduction on a scoreboard sample before adding new semantic coverage.
