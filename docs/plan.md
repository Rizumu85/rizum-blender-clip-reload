...
- HSL filter payload mapping is now sample-backed separately from the native
  ...
  routine.
- Large-stack performance has passed the sparse-tile milestone, atlas-backed pass-collapse, direct compressed raster tile chunks, and the first masked Normal atlas-run subset. The host-facing normal render path uses a recursive streaming GPU source provider: the main selector builds a metadata-only GPU source tree/resource plan, then raster/mask payloads are decoded and uploaded at point of use inside containers, clipping runs, THROUGH groups, and filters. CHNKExta tile-blob parsing is split into `clip_file::external`; compressed tile blocks inflate directly into the expected output buffer, selected-region reads stream matching external blocks through reusable scratch storage, and region raster/mask reads skip empty tile blocks even when the requested source rectangle covers the full metadata extent. Runtime providers precompute compressed-present tile bounds, expose the cropped offset/size to streaming bounds, and mark all-empty raster sources as known empty before decode/upload. For eligible raster runs, runtime now decodes non-empty compressed raster tiles directly into atlas chunk events, uploads only those chunks, and renders the run through one tile-silo shader pass; masked Normal runs may participate when the runtime provider pre-applies layer-mask alpha into the atlas chunks with the existing integer Normal mask rule. The older per-source texture-copy path remains for providers that do not expose tile chunks, but it is not used for masked atlas runs because that would skip mask application. Raster source shaders sample outside cropped uploads as transparent black while generated/cache textures keep transparent white, preserving `.clip` empty-raster semantics without expanding cache bounds back to canvas-sized metadata. Clipping-run nodes, filters, THROUGH groups, containers as source nodes, masked standard blends, and byte-domain special blends remain semantic barriers on the existing faithful pass path. Direct release executable checks on this machine are stable: `Test_RealArt.clip` compares at `raw_max=5` with current direct-run `premul_max=1`, and `Ref_Terra404_Live2D.clip` renders with `premul_max=3` / `premul_visible_px=13501`; worker timings remain in the same range as the earlier RealArt/Terra measurements and are not yet the full Silicate-style mask model. The next native performance step is true tile-local mask/clipping events and larger faithful raster/clipping segments; do not introduce CPU compositor fallback, post-processing, or a global all-layer full-canvas texture cache.
...
- High-leverage native performance work has moved from broad investigation to a
  validated sparse-tile implementation plus atlas raster-run collapse and
  direct compressed raster tile chunks.
  ...
  events instead of first uploading per-source textures. Masked Normal rasters
  can join those provider-backed atlas runs by pre-applying mask alpha to
  decoded atlas chunks; masked standard blends, clipping relationships, and
  containers remain barriers.
  A follow-up source review of
  Avarel/silicate confirms that the remaining order-of-magnitude milestone is
  richer mask/clipping tile events and larger tile-local raster/clipping
  segments while keeping filters, THROUGH groups, and isolated containers as
  explicit semantic barriers until faithful tile-local models exist. Short-lived
  ...
  extend tile-local mask/clipping events on the native side, and use Blender
