...
- Surface native support summaries in Blender from the metadata-only C ABI
  ...
  reports; show compact unsupported layer/node/kind locators and allow copying
  just those locations for source-layer follow-up.
...
- Current native milestone: the stock Blender image-datablock bridge is the
  ...
  preference is an override only. The project-root `clip_loader.py` remains slow
  reference tooling for verification while native fidelity gaps close.
