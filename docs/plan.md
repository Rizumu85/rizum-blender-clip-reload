...
- `clip_cli --gpu-trace-pixel <x> <y>` is available for native GPU prefix tracing and now includes per-source before/after/input pixels. `IllustrationBlendModes.clip` now keeps a one-LSB Subtract blend target for partially transparent equal source/destination channels before Color Dodge/Color Burn, reducing that sample to `raw_max=7`. `IllustrationBlendModes2.clip` now also floors partial Hue writeback, fixing one former red hot spot, but the main open blend trace remains the `(427,138)` Pin Light/Hue/Saturation residual; rejected broad fixes such as global `256-srcA`, HSL tiny-span `3/255`, Rec.601 HSL luma, Hue min-channel ceil, PinLight byte-domain/256-alpha writeback, and standard-pass partial-alpha decrement remain evidence only.
