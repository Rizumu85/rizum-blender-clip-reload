...
- Current native milestone: promote the stock Blender image-datablock bridge from
  ...
  making native mode the accepted/default Blender import path, and then deleting
  the Python compositor/loader and sidecar workflow once the native path owns the
  whole user workflow. The installable add-on zip is built by
  ...
  discovery; the preference path is now an override.
