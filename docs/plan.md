...
- HSL filter payload mapping is now sample-backed separately from the native
  ...
  routine.
- Large-stack performance has passed the sparse-tile milestone, atlas-backed pass-collapse, direct compressed raster tile chunks, masked Normal atlas-run support, shader-side R8 mask atlas events for masked standard raster runs, a conservative clipped-raster-sibling tile-silo subset inside already-created clipping caches, and a raster-only clipping-run tile-silo subset where base/clipped relationships are encoded as tile-local events. The host-facing normal render path uses a recursive streaming GPU source provider: the main selector builds a metadata-only GPU source tree/resource plan, then raster/mask payloads are decoded and uploaded at point of use inside containers, clipping runs, THROUGH groups, and filters. CHNKExta tile-blob parsing is split into `clip_file::external`; compressed tile blocks inflate directly into the expected output buffer, selected-region reads stream matching external blocks through reusable scratch storage, and region raster/mask reads skip empty tile blocks even when the requested source rectangle covers the full metadata extent. Runtime providers precompute compressed-present tile bounds, expose the cropped offset/size to streaming bounds, and mark all-empty raster sources as known empty before decode/upload. For eligible raster runs, eligible consecutive raster clipped siblings, and eligible raster-only clipping runs, runtime now decodes non-empty compressed raster tiles directly into atlas chunk events, optionally emits matching R8 mask atlas chunks, uploads only those chunks, and renders the segment through one tile-silo shader pass. Raster-only clipping-run mode composites base events into a tile-local clip destination, applies clipped events with preserve-alpha semantics, and resolves the local result through the base blend mode. Provider chunks outside cropped cache/target bounds are filtered before event emission. The older per-source texture-copy path remains for providers that do not expose tile chunks. Raster source shaders sample outside cropped uploads as transparent black while generated/cache textures keep transparent white, preserving `.clip` empty-raster semantics without expanding cache bounds back to canvas-sized metadata. Filters, THROUGH groups, containers as source nodes, clipped container/folder siblings, and byte-domain special blends remain semantic barriers on the existing faithful pass path. Direct release executable checks on this machine are stable: `Test_RealArt.clip` compares at `raw_max=5` with current direct-run `premul_max=1`, and `Ref_Terra404_Live2D.clip` renders with `premul_max=3` / `premul_visible_px=13619`. The next native performance step is broader tile-local semantic event support across the remaining barriers; do not introduce CPU compositor fallback, post-processing, or a global all-layer full-canvas texture cache.
...
- The durable performance roadmap is now `docs/native-tile-event-renderer-roadmap.md`: tile-local event execution should become the main renderer model, while barrier passes are explicit, counted, and explained as not-yet-lowered faithful semantics. The next work should start with performance-plan JSON and explainable `BarrierReason`s, then move through typed tile events, byte-domain special blend events, pointwise filter events, tile-local scope stacks, and only then session-level sparse atlas caching plus dirty segment reload.
