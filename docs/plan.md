...
- Current native milestone: promote the stock Blender image-datablock bridge from
  ...
  images through manual reload and the background watcher without writing
  sidecar PNGs. Remaining functional work is making this native mode the
  accepted/default Blender import path, adding load-time source freshness scans,
  and then deleting the Python compositor/loader and sidecar workflow once the
  native path owns the whole user workflow.
