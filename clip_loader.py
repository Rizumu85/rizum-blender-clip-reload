...
def _blend_func(mode: str, s: np.ndarray, d: np.ndarray) -> np.ndarray:
    ...
    if mode == "NORMAL" or mode == "THROUGH":
        return s
    if mode == "MULTIPLY":
        return s * d
    if mode == "SCREEN":
...
class ClipFile:
    ...
    def _composite_image(self, out: np.ndarray, layer: sqlite3.Row, rgba: np.ndarray,
        ...
        # layer to match CSP's precision and avoid float error accumulation
        # across many stacked layers.
        out[y0:y1, x0:x1] = np.clip(np.floor(out[y0:y1, x0:x1] * 255.0 + 0.5), 0, 255) / 255.0
    ...
    def _composite_clipped_image(
        ...
        if always_preserve:
            ...
            # CSP's IsBelowClipping (offset 0x1c8) is binary.
            # The clip mask (_apply_mask_and_clip) already constrains alpha.
            # Preserve handles the edge case where the clip base IS the
            # visible surface; regular Porter-Duff handles the case where
            # opaque artwork already exists below the clip base.
        ...
        while i < len(chain_ids):
            ...
            if layer["LayerType"] == LAYER_TYPE_LAYER_FOLDER:
                ...
                if mode == "THROUGH":
                    ...
                    # Normal folder is just an organizational container;
                    # children render directly on the current canvas.
                    self._render_chain(layer["LayerFirstChildIndex"], out, preserve_clipped_alpha)
                else:
                    # Non-Normal folder renders children on white paper,
                    # then composites the result through its blend mode.
                    group_out = np.ones_like(out)
            ...
            if layer["LayerType"] == LAYER_TYPE_GROUP:
                ...
                if mode == "THROUGH":
                    ...
                    # Normal group renders children directly on the current canvas.
                    self._render_chain(layer["LayerFirstChildIndex"], out, preserve_clipped_alpha)
                else:
                    # Non-Normal group renders children on white paper,
                    # then composites the result through its blend mode.
                    group_out = np.ones_like(out)
                    self._render_chain(layer["LayerFirstChildIndex"], group_out, True)
                    rgba, _ = self._premul_to_rgba_u8(group_out)
