...
#[test]
fn simple_container_scope_with_nested_container_clipping_run_lowers_ordered_tile_events() {
    let plan = sparse_atlas_raster_event_plan(
        &diff_with_segment(segment("SimpleContainerScope")),
        &reload_with_slots(vec![
            slot("raster", 10, 1, 0, 12, 34),
...
#[test]
fn simple_container_scope_with_clipped_container_sibling_lowers_ordered_tile_events() {
    let plan = sparse_atlas_raster_event_plan(
        &diff_with_segment(segment("SimpleContainerScope")),
        &reload_with_slots(vec![
            slot("raster", 10, 1, 0, 12, 34),
...
#[test]
fn clipped_container_sibling_with_point_filter_lowers_ordered_tile_events() {
    let plan = sparse_atlas_raster_event_plan(
        &diff_with_segment(segment("SimpleContainerScope")),
        &reload_with_slots(vec![
            slot("raster", 10, 1, 0, 12, 34),
    ...
    assert_eq!(batch.events.len(), 2);
    assert_eq!(batch.filters.len(), 1);
    assert_eq!(batch.tile_events.len(), 7);
    assert!(matches!(
        batch.tile_events[0],
        clip_gpu::GpuSparseAtlasTileEvent::BeginClipBase(_)
    ));
    ...
    assert!(matches!(
        batch.tile_events[4],
        clip_gpu::GpuSparseAtlasTileEvent::PointFilter(_)
    ));
    assert!(matches!(
        batch.tile_events[5],
        clip_gpu::GpuSparseAtlasTileEvent::EndClippedScope(_)
...
#[test]
fn clipped_container_sibling_with_nested_container_lowers_ordered_tile_events() {
    let plan = sparse_atlas_raster_event_plan(
        &diff_with_segment(segment("SimpleContainerScope")),
        &reload_with_slots(vec![
            slot("raster", 10, 1, 0, 12, 34),
...
#[test]
fn clipped_container_sibling_with_child_clipping_run_lowers_ordered_tile_events() {
    let plan = sparse_atlas_raster_event_plan(
        &diff_with_segment(segment("SimpleContainerScope")),
        &reload_with_slots(vec![
            slot("raster", 10, 1, 0, 12, 34),
            slot("raster", 11, 2, 1, 12, 34),
            slot("raster", 12, 3, 2, 12, 34),
        ]),
        &[clip_gpu::GpuNormalStackSource::Container {
            children: vec![clip_gpu::GpuNormalStackSource::ClippingRun {
                base: raster_source(10, 1, 1.0, clip_gpu::GpuRasterBlendMode::Normal, None),
                clipped: vec![clip_gpu::GpuClippedStackSource::Container {
                    layer_id: LayerId(11),
                    children: vec![clip_gpu::GpuNormalStackSource::ClippingRun {
                        base: raster_source(11, 2, 1.0, clip_gpu::GpuRasterBlendMode::Normal, None),
    ...
    assert_eq!(batch.tile_events.len(), 9);
    assert!(matches!(
        batch.tile_events[0],
        clip_gpu::GpuSparseAtlasTileEvent::BeginClipBase(_)
    ));
...
fn clipped_container_sibling_with_through_child_clipping_run_lowers_ordered_tile_events() {
    ...
    let batch = &plan.segments[0].batches[0];
    assert_eq!(batch.events.len(), 3);
    assert_eq!(batch.tile_events.len(), 11);
    assert!(matches!(
        batch.tile_events[0],
        clip_gpu::GpuSparseAtlasTileEvent::BeginClipBase(_)
    ));
    ...
    assert!(matches!(
        batch.tile_events[10],
        clip_gpu::GpuSparseAtlasTileEvent::ResolveClipBase(_)
    ));
}

#[test]
fn clipped_container_sibling_with_through_child_then_filter_lowers_ordered_tile_events() {
    let plan = sparse_atlas_raster_event_plan(
        &diff_with_segment(segment("SimpleContainerScope")),
        &reload_with_slots(vec![
            slot("raster", 10, 1, 0, 12, 34),
...
#[test]
fn clipped_container_sibling_with_nested_container_through_child_lowers_ordered_tile_events() {
    let plan = sparse_atlas_raster_event_plan(
        &diff_with_segment(segment("SimpleContainerScope")),
        &reload_with_slots(vec![
            slot("raster", 10, 1, 0, 12, 34),
