...
Linux x64 and macOS packages are built as separate zips and should be treated
as test candidates until they pass real-device checks.
...
`clip_cli` and `libclip_capi.so`; macOS packages contain `clip_cli` and
`libclip_capi.dylib`.
...
会作为单独 zip 构建，真实设备检查通过前先当作测试候选包。
...
`clip_cli` 和 `libclip_capi.so`；macOS 包里是 `clip_cli` 和 `libclip_capi.dylib`。
