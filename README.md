...
The current release package is for Windows x64. Linux and macOS builds will be
published later as separate platform-specific packages after real-device
...
If several native platforms are prepared later, build each platform as its own
zip:

```powershell
python tools\build_blender_addon.py --platform linux-x64 --output clip_studio_importer-linux-x64.zip --blender "C:\path\to\blender.exe"
python tools\build_blender_addon.py --platform macos-arm64 --output clip_studio_importer-macos-arm64.zip --blender "/Applications/Blender.app/Contents/MacOS/Blender"
```

Each generated zip should contain only its matching `native/<platform>/` files
and should be uploaded as its own platform version on Blender Extensions. The
`Build extension package` GitHub Actions workflow currently uploads only the
Windows x64 package.
...
当前 release package 只面向 Windows x64。Linux 和 macOS 会等真实设备测试或用户反馈后，
再作为单独平台包发布。
...
以后如果准备多个 native 平台，请每个平台单独构建一个 zip：
...
每个生成的 zip 都应该只包含对应的 `native/<platform>/` 文件，并在 Blender Extensions
上作为对应平台的独立版本上传。当前 `Build extension package` GitHub Actions workflow
只上传 Windows x64 包。
