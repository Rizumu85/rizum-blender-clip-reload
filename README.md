...
Release packages are split by platform. Windows x64 is maintainer-tested.
Linux x64 and macOS Apple Silicon packages are built as separate zips and
should be treated as test candidates until they pass real-device checks.
...
- Simple text layers as flattened native pixels.
...
Future plan:
- Better shaped text parity for mixed-script edge cases.
...
By default the script packages the current host platform. For Blender
Extensions, upload one platform package at a time. Do not upload a universal zip
that contains native files for several operating systems.

```powershell
python tools\build_blender_addon.py --platform windows-x64 --output clip_studio_importer-windows-x64.zip
```

To build every supported platform from collected native artifacts, write one zip
per platform:
...
Each generated zip contains only its matching `native/<platform>/` files and
should be uploaded as its own platform version on Blender Extensions. Windows
packages contain `clip_cli.exe` and `clip_capi.dll`; Linux packages contain
`clip_cli` and `libclip_capi.so`; macOS Apple Silicon packages contain
`clip_cli` and `libclip_capi.dylib`.

The `Build extension package` GitHub Actions workflow builds native artifacts
on Windows, Linux, and macOS runners, then uploads split platform zips from
`dist/`.
...
Release package 会按平台拆开。Windows x64 已在维护者机器上测试；Linux x64 和 macOS
Apple Silicon 会作为单独 zip 构建，真实设备检查通过前先当作测试候选包。
...
- 常见混合模式。
- 当前验证样本覆盖的调整图层和滤镜图层。
- 简单文字图层会以 flattened native pixels 渲染。
- 手动重载和非阻塞自动重载。
- 图片结果打包保存进 `.blend`。
- 缺失源文件、渲染错误、pack 状态等清晰提示。

以后补充：
- 混合文字/复杂排版边缘情况的 shaped text 细节。
...
默认情况下脚本会打包当前主机平台。提交 Blender Extensions 时，请一次上传一个平台包，
不要上传同时包含多个操作系统 native 文件的 universal zip。

```powershell
python tools\build_blender_addon.py --platform windows-x64 --output clip_studio_importer-windows-x64.zip
```

如果已经收集好多个 native 平台的 artifacts，可以一次写出多个独立 zip：

```powershell
python tools\build_blender_addon.py --platform all --output-dir dist
```

每个生成的 zip 都只包含对应的 `native/<platform>/` 文件，并在 Blender Extensions 上作为
对应平台的独立版本上传。Windows 包里是 `clip_cli.exe` 和 `clip_capi.dll`；Linux 包里是
`clip_cli` 和 `libclip_capi.so`；macOS Apple Silicon 包里是 `clip_cli` 和
`libclip_capi.dylib`。

`Build extension package` GitHub Actions workflow 会在 Windows、Linux、macOS runner 上
构建 native artifacts，然后从 `dist/` 上传拆分后的平台包。
